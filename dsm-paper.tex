...
%In the above form, the model implies that only group abundance can be calculated if data have been collected on groups or clusters of animals. This limitation can be alleviated by using a Horvitz-Thompson-type estimator of individual abundance and using this as the response.
...
\end{itemize}
%[[tricky/time-consuming \textbf{don't think we have time for any of this}:
%\begin{itemize}
...
%\subsubsection*{Markov modulated Poisson processes}
%
%It is often the case in line transect surveys that clusters of animals or groups are observed. If such patterns are merely due to short-term social behaviour or transient prey density, rather than long-term environmental factors then this can lead to incorrect inference (since those areas identified as hot spots are simply short-term agglomerations).
%
%\cite{Skaug:2006gs} suggested the use of a Poisson process for modelling line transect data, however with the Poisson process' intensity function modulated using a finite state Markov process, where the states can be interpreted biologically. For example, with a two state process one can think of switching between high and low encounter rates which could correspond to, say a ``feeding'' state and ``transit'' state. The intensity can also be written as a function of spatially referenced covariates as above, via a GAM or GLM setup. The MMPP accounts only for unidimensional correlation along the transect line, not overall spatial autocorrelation.
